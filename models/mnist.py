...
#         self.x_discriminator = sn(nn.Linear(hidden_dim, 1))
#
#         layers = [
...
#             ]
#
#         self.z_discrimination = nn.Sequential(*layers)
#         self.z_discriminator = sn(nn.Linear(hidden_dim, output_dim))
#
#         layers = [
#             sn(nn.Linear(2 * hidden_dim, 2 * hidden_dim)),
#             nn.LeakyReLU(inplace=True),
#
#             sn(nn.Linear(2 * hidden_dim, output_dim)),
#         ]
#
...
#
#
# class XZDiscriminatorSeparate(nn.Module):
#     def __init__(self, latent_dim,
#                  img_shape=(1, 28, 28), hidden_dim=400, extra_layers=0,
#                  output_dim=1, spectral_norm=True):
#         super().__init__()
...
#         #     sn(nn.Linear(2 * hidden_dim, output_dim)),
#         # )
#         # self.discriminator = sn(nn.Bilinear(hidden_dim, hidden_dim, bias=True, out_features=output_dim))
#
#     def forward(self, x, z):
#         hx = self.x_discrimination(x)
#         hz = self.z_discrimination(z)
#         # out = self.discriminator(torch.cat([hx, hz], dim=1))
#         out = torch.sum(self.fc(hx) * hz, dim=-1, keepdim=True)
#
#         return out
...
#
#
# class XZFeatureDiscriminator(nn.Module):
#     def __init__(self, x_feature, z_feature, input_dim=512, output_dim=1, spectral_norm=True):
#         super().__init__()
#
#         self.x_feature = x_feature
